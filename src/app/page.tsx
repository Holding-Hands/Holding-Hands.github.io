...
function HomePage() {
  ...
  const [selectedGuide, setSelectedGuide] = useState<string | null>(searchParams.get('id'))
  ...
  if (selectedGuide) {
    ...
    // If guide id in URL is invalid, it will just show nothing or error, maybe fallback?
    // For now we assume valid ID or just let it render (it might crash if guide is undefined)
    if (guide) {
      return (
        <GuideViewer
          guide={guide}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
          <div className="flex items-center justify-between min-h-[44px]">
            ...
            <div className="flex-1 flex items-center justify-start sm:justify-center min-w-0 mr-10 sm:mr-0 gap-2 sm:gap-3">
              <img
                src="https://s3.bmp.ovh/imgs/2025/11/27/c09f0f0f73216b4f.png"
                alt="Logo"
            ...
            <div className="hidden sm:flex absolute right-4 sm:right-6 lg:right-8 items-center gap-2">
              <button
                onClick={toggleTheme}
            ...
            <div className="sm:hidden absolute right-4 top-1/2 transform -translate-y-1/2 z-20">
              <button
                onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
                className="p-2 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors"
              >
                <div className="w-6 h-6 relative flex flex-col justify-center items-center">
                  <span className={`block absolute h-0.5 w-5 bg-current transform transition-all duration-300 ease-in-out ${isMobileMenuOpen ? 'rotate-45' : '-translate-y-1.5'}`}></span>
                  <span className={`block absolute h-0.5 w-5 bg-current transform transition-all duration-300 ease-in-out ${isMobileMenuOpen ? 'opacity-0 scale-0' : 'opacity-100'}`}></span>
                  <span className={`block absolute h-0.5 w-5 bg-current transform transition-all duration-300 ease-in-out ${isMobileMenuOpen ? '-rotate-45' : 'translate-y-1.5'}`}></span>
                </div>
              </button>
            </div>
          ...
          <div className={`sm:hidden absolute top-full left-0 right-0 bg-white/95 dark:bg-gray-800/95 backdrop-blur-md border-b border-gray-200 dark:border-gray-700 shadow-lg z-10 transition-all duration-300 ease-out origin-top ${isMobileMenuOpen
            ...
            <div className="px-4 py-4 grid grid-cols-3 gap-4">
              <button
                ...
                className="flex flex-col items-center gap-2 p-3 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl transition-colors"
              >
                {theme === 'dark' ? (
